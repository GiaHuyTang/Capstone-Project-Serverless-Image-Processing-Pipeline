...
This project implements a fully serverless image processing pipeline on AWS. The application is designed to automatically resize an original image to a thumbnail size upon external request, demonstrating proficiency in integrating multiple core AWS services.

The application workflow is orchestrated by a State Machine and is triggered via an API Gateway endpoint.
...
2.  **API Gateway** uses a VTL Mapping to trigger the **Step Functions State Machine**.
3.  The **Step Functions** state machine invokes the **AWS Lambda Function**.
4.  The **Lambda Function** retrieves the original image from **S3 Bucket (Original)**, processes the resize operation, and stores the resulting thumbnail in **S3 Bucket (Resized)**.
...
* **Lambda Layer:** A Lambda Layer containing the native dependencies for image processing (e.g., the **Pillow** library for Python) must be created and attached to the Lambda Function.
